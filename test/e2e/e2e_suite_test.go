...
// Package e2e contains the end-to-end test suite for cluster-api-provider-maas.
//
// The suite follows the pattern used by the upstream CAPI providers (e.g.
// cluster-api-provider-vsphere): a single flat suite that boots a kind based
// management cluster, builds a local clusterctl repository from the providers
// declared in the e2e config, and then drives the workload cluster lifecycle by
// reusing the shared spec library in sigs.k8s.io/cluster-api/test/e2e.
//
// The whole package is guarded by the "e2e" build tag so it is excluded from the
// default build and from `make test` (which runs the Ginkgo v1 unit suites).
package e2e
...
var (
	...
	// useExistingCluster instructs the test to use the current cluster instead of creating a new one.
...
var (
	...
	// Provisioning will be skipped if e2e.use-existing-cluster is provided.
...
// The bootstrap cluster is shared across all the tests, so it should be deleted only after all ParallelNodes complete.
var _ = SynchronizedAfterSuite(func() {
	...
	dumpBootstrapClusterLogs(ctx, artifactFolder, bootstrapClusterProxy)
...
// Byf is a helper that wraps ginkgo.By with fmt.Sprintf formatting.
func Byf(format string, a ...interface{}) {
	By(fmt.Sprintf(format, a...))
}

// initScheme returns a runtime.Scheme with all the GVKs relevant for the MAAS e2e tests:
// the default CAPI schemes (core, kubeadm bootstrap/control-plane, addons/CRS, ...) plus the
// MAAS infrastructure types (MaasCluster, MaasMachine, MaasMachineTemplate).
func initScheme() *runtime.Scheme {
	sc := runtime.NewScheme()
	framework.TryAddDefaultSchemes(sc)
	Expect(infrav1.AddToScheme(sc)).To(Succeed(), "Failed to add the MAAS infrastructure types to the scheme")
	return sc
}
...
func createClusterctlLocalRepository(config *clusterctl.E2EConfig, repositoryFolder string) string {
	...
	// Ensure a CNI file is defined in the config and register a FileTransformation to inject the referenced
	// file content in place of the ${CNI_RESOURCES} envSubst variable used by the ClusterResourceSet ConfigMap.
	...
	Expect(clusterctlConfig).To(BeAnExistingFile(), "The clusterctl config file does not exist in the local repository %s", repositoryFolder)
...
func setupBootstrapCluster(config *clusterctl.E2EConfig, scheme *runtime.Scheme, useExistingCluster bool) (bootstrap.ClusterProvider, framework.ClusterProxy) {
	...
	if !useExistingCluster {
		...
		clusterProvider = bootstrap.CreateKindBootstrapClusterAndLoadImages(ctx, bootstrap.CreateKindBootstrapClusterAndLoadImagesInput{
			...
			IPFamily:           config.MustGetVariable(capi_e2e.IPFamily),
...
// dumpBootstrapClusterLogs collects the machine logs of the kind based bootstrap cluster nodes.
func dumpBootstrapClusterLogs(ctx context.Context, artifactFolder string, clusterProxy framework.ClusterProxy) {
	if clusterProxy == nil {
		return
	}
