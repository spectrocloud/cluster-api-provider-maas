...
func TestMaasClusterConversion(t *testing.T) {
	g := NewWithT(t)
...
func TestMaasMachineConversion(t *testing.T) {
	g := NewWithT(t)
	ns, err := testEnv.CreateNamespace(ctx, fmt.Sprintf("conversion-webhook-%s", util.RandomString(5)))
	g.Expect(err).ToNot(HaveOccurred())
...
func TestMaasMachineTemplateConversion(t *testing.T) {
	g := NewWithT(t)
	ns, err := testEnv.CreateNamespace(ctx, fmt.Sprintf("conversion-webhook-%s", util.RandomString(5)))
	g.Expect(err).ToNot(HaveOccurred())
	machineTemplateName := fmt.Sprintf("test-machine-%s", util.RandomString(5))
	machine := &MaasMachineTemplate{
		ObjectMeta: metav1.ObjectMeta{
